...
const NAV_ITEMS: NavItemDef[] = [
  { href: '/', icon: <Github className="h-4 w-4" />, label: 'Checkout', active: true },
  { href: '/history', icon: <History className="h-4 w-4" />, label: 'History' },
  { href: '/terminal', icon: <Terminal className="h-4 w-4" />, label: 'Terminal' },
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        >
          {!collapsed && (
            <>
              <div className="flex items-center justify-center h-7 w-7 rounded-md bg-accent text-accent-foreground text-xs font-bold shrink-0">
                G
              </div>
              <span className="flex-1 font-semibold text-sm truncate">GH Checkout</span>
            </>
          )}
          <Tooltip>
        ...
        >
          ...
          {collapsed ? (
            <Tooltip>
              ...
              <TooltipContent side="right">User · {user?.email}</TooltipContent>
            </Tooltip>
          ) : (
